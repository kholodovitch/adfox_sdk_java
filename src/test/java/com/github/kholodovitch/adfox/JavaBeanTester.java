...
public class JavaBeanTester {
		...
		nextProp: for (PropertyDescriptor prop : props) {
			...
			if (getter != null && setter != null) {
				...
				if (params.length == 1 && params[0] == returnType) {
					// The set method has 1 argument, which is of the same type
					// as the return type of the get method, so we can test this
					// property
					try {
						// Build a value of the correct type to be passed to the
						// set method
						Object value = buildValue(returnType);

						// Build an instance of the bean that we are testing
						// (each property test gets a new instance)
						T bean = clazz.newInstance();

						// Call the set method, then check the same value comes
						// back out of the get method
	...
	private static Object buildValue(Class<?> clazz) throws InstantiationException, IllegalAccessException, IllegalArgumentException, SecurityException, InvocationTargetException {
		...
		if (clazz == String.class) {
			...
			fail("Unable to build an instance of class " + clazz.getName() + ", please add some code to the " + JavaBeanTester.class.getName() + " class to do this.");
			return null; // for the compiler
		}
	}

	/**
	 * Hunt down missing Boolean read method if needed as Introspector cannot
	 * find 'is' getters for Boolean type properties.
	 * 
	 * @param clazz
